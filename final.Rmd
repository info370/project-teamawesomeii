...
```{r}
# drinks linear model
drinks_linear_model <- lm (GPA ~ Drinks, data = sleep_data)
drinks_parameters <- tidy(drinks_linear_model)$estimate
drinks_parameters
```
...
sleep_data_small <- sleep_data %>%
  ...
  mutate(drinks_lm_prediction = drinks_parameters[1] + drinks_parameters[2] * sleep_data$Drinks)

ggplot(sleep_data_small , aes(Drinks)) +
  geom_point(aes(y= GPA), size = 1) +
  geom_line(aes(y= drinks_lm_prediction), size = 1, colour = "red")
```


```{r}
drinks_lm_function <- function(a, data) {
  a[1] + data$Drinks * a[2]
}

drinks_measure_distance <- function(mod_params, data) {
  diff <- data$GPA - drinks_lm_function(mod_params, data)
  sqrt(mean(diff ^ 2))
}

# line fit with squares of the residuals function
drinks_best <- optim(c(3.4, 0), drinks_measure_distance, data = sleep_data_small)
...
drinks_mad_fit <- optim(c(3.4, 0), drinks_measure_distance_mad, data = sleep_data_small)
drinks_mad_fit$par
```
...
sleep_data_small <- sleep_data_small %>% 
  mutate(
    ...
    residuals = GPA - drinks_lm_prediction
  )
...
<summary of absences vs gpa for all datasets>
